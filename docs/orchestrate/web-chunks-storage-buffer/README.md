...
- [x] **Validation-error fix dispatch** — 4 mechanical edits, all 3 named errors resolved. (1) `map_copy.wgsl` `old_map` storage promoted `read` → `read_write` (WGSL atomic-in-storage spec). (2) `map_copy.rs` layout entry flipped. (3) `generator_model.wgsl` entry-point renamed `fill_chunk_data_with_model_data_16` → `fill_chunk_data_with_model_data` (naga WGSL-writeback trailing-digit-suffix bug). (4) `generator_model.rs` caller updated. **Committed at `c8a3450`.**
- [x] **Hard-gate user review** — user said "commit".
- [ ] **Pending direction**: (a) declare orchestration done, (b) extend into the residual headless `DeviceLost`, (c) tack on the trivial 404-favicon cleanup before closing.
