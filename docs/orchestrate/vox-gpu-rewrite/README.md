...
- [x] Stage 2 committed `71588ac`
- [x] Stage 12 diagnostic — `17-diagnostic-residual-speckle-and-brush-clears.md` identified TWO separate bugs:
  - **Bug 1 (HIGH confidence): `--small-edit-repro` brush-clears** — `seed_block_hashing` calls `BlockHashingHandler::add_block` which APPENDS duplicate voxel data + returns END pointer (10.5M-17M range). GPU's voxels[] only has data in 0-10.5M range. At edit time, every unmodified Mixed block's hash matches → returns END_PTR → GPU writes END_PTR into blocks[] → renderer reads voxels[END_PTR/2+...] = 0 → block renders empty. **Real production bug.** Fix: add `BlockHashingHandler::seed_block(hash, payload, existing_ptr)` that registers an existing slot without appending; use it from `seed_block_hashing` instead of `add_block`.
  - **Bug 2 (MEDIUM confidence): `--vox-gpu-oracle` per-pixel ceiling** — test calibration: CPU phase uses sized-to-model (1488×544×1344, no tiling), GPU phase uses fixed-world (4096×512×4096 with tiling + Y-clamp). Semantically different worlds → secondary GI rays hit different geometry → 6% per-pixel divergence. **Test artifact, not runtime bug.** Fix: tighten oracle viewport OR replace with GPU-vs-GPU compare.
- [ ] Hard gate — submit Stage 12 diagnostic; pending Bug 1 fix + Bug 2 test fix  ← CURRENT
