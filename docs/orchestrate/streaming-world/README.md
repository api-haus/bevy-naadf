...
- [x] 04g — Phase-2.14.g: e2e gate regression sweep. Three gates run once each, stop-on-first-failure per agent brief. Catches regressions from the 2.14.b/c/d/e/f bottom-up refactor in the live runtime path. **Results:** `--gate streaming-cold-start` PASS (14/14 camera-row segments non-empty, cold-start complete at frame 259); `--gate streaming-window` PASS (pixel-Δ 46.22 / floor 3.00, variance 2341.60 / floor 800.00, X-axis origin shift = 4 / floor 4, max per-frame walk 22 ms / cap 50 ms, mid-walk non-sky 0.785 / floor 0.300); `--gate oasis-edit-visual` PASS (rect mean per-pixel RGB Δ 17.93 / floor 8.00 on a 30-voxel erase sphere). Total wall-clock ~30 s end-to-end. See [`04g-impl-e2e-regression-sweep.md`](./04g-impl-e2e-regression-sweep.md).
- [x] **Hard gate (Phase 2.14.g)** — GREEN: `streaming-cold-start` PASS / `streaming-window` PASS / `oasis-edit-visual` PASS. Phase 2.14 bottom-up refactor preserved runtime behaviour across the three e2e regression catchers. Live-binary interactive UX remains the user's eyeball job per `bevy-naadf/CLAUDE.md`.
