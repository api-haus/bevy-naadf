...
- [⚠] `04F` — D4 follow-up + Resolution D — **Resolution D landed (flat absorption: `ConstructionPipelines` fields merged into `NaadfPipelines`); Step 6 WorldGpu consolidation landed; pbr_sampling.wgsl deleted (−868 LOC alone)**. Net −893 LOC. Build ✓ / 179 lib tests pass / 6 e2e gates green incl oasis-edit-visual ×4.
- [⚠] `04F` — D4 final cleanup — **Step 4 (prepare.rs split into `prepare/{mod.rs,frame.rs,world.rs}`) + alias rename + production→e2e dep-arrow inversion (`demo_origin_v` moved out of `e2e::gates`) landed**. Net +43 LOC (prepare.rs module-header docs). Build ✓ / 179 lib tests pass / 6 e2e gates green incl oasis-edit-visual ×3. **Step 5 plugin-per-subsystem deferred again (budget)**. **New surfacing**: `window_config.rs` has same production→e2e dep-arrow anti-pattern as `demo_origin_v` — needs sweep-or-not decision.
