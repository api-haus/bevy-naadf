...
| `03f-impl-visual-diff-edit-fix.md` | `02f`-followup — visual-diff e2e gate (`--oasis-edit-visual`) catching the consolidated-rearch's user-visible edit regression + the runtime fix. Root cause: main-world `Last` schedule clear of `pending_edits.batches` raced ahead of the render sub-app's `ExtractSchedule`. Fix: drain inside `extract_world_changes` via `ResMut<MainWorld>`. Also bumped four `W2_CHANGED_*_INIT` static caps that overflowed at Oasis brush sizes (silently dropping wgpu writes). | **complete** (general-purpose Opus 4.7 (1M context), 2026-05-15) — uncommitted on disk |
| `03g-impl-small-edit-fix.md` | Single-voxel-edit gate (`--small-edit-visual`) closing the regression hole below `--oasis-edit-visual`'s scale. CPU snapshot assertion (`count_non_empty_voxels` before vs after = +1) catches Mode-2 encoder-phantom bugs; framebuffer assertions (click-rect max-pixel-delta + adjacent-rect mean + whole-frame catastrophic-fraction) catch Mode-1 AADF-staleness / W2 buffer-overflow / global rendering corruption. 184 lib tests pass (+4 reproducer unit tests confirming the CPU encoder is bit-correct for the user's reported Mode-2 scenario). All 9 e2e modes PASS. Conclusion: neither user-reported mode reproducible in the port's edit chain as a real bug; gate is permanent regression coverage. | **complete** (general-purpose Opus 4.7 (1M context), 2026-05-15) — uncommitted on disk |
