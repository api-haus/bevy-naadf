...
- **Phase-C followups dispatched:** consolidated cleanup of all 5 concerns + 1 nit (wire the producer flip; add drift guard for inline shader copies; reconcile a doc; guard `entity_instances_history`; calibrate `--entities` pixel gate; fix stale `prepare.rs` doc).
- **Phase-C feature-complete.** Paper methodology §3.2–3.6 fully implemented and verified; all 7 workstreams (W0/W6/W5/W1/W3/W4/W2 + wave-3 integration) shipped clean.
