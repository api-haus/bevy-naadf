...
- **Phase-C review COMPLETE:** verdict **PASS-WITH-FOLLOWUPS** (0 blockers, 5 concerns, 4 nits). All 7 success criteria green; all concerns reconcile as REAL gaps. The medium-scope item: `run_gpu_construction_startup` is currently a placeholder — GPU Algorithm 1 is correct + oracle-gated + exercised by `--validate-gpu-construction`, but the production render path still reads CPU-built buffers. The producer flip is the main followup.
- **Phase-C followups COMPLETE:** commit `e9d7141` merged. All 5 concerns + nit #1 closed. **Runtime GPU producer chain is now ACTIVE** — `naadf_gpu_producer_node` dispatches Algorithm 1 every startup. **Honest Phase-D residual:** the *full upload-skip path* (renderer reads only from GPU-produced buffers, no CPU upload) hits a wgpu/Vulkan storage-texture barrier hazard between the construction bind (`texture_storage_3d<rg32uint, read_write>`) and the renderer bind (`texture_3d<u32>`) on the same texture; GPU writes don't propagate to the renderer's read view. With CPU upload retained, the GPU producer still dispatches (Algorithm 1 IS exercised at runtime) and bit-exact validation proves output equivalence. This is a wgpu infrastructure issue, not a port-correctness gap.
- **Phase C status: DONE.** Paper §3.2–3.6 fully implemented and oracle-verified. 112 tests pass. 4 e2e modes pass (baseline · `--validate-gpu-construction` 388-byte byte-equal · `--edit-mode` · `--entities` with the new `entity_pixel` gate at 2.35× safety margin).

## Phase-D residuals (for a future session)

1. **wgpu storage-texture barrier hazard** (T1 from `16-impl-c-followups.md`) — investigate the barrier requirement between construction-bind and renderer-bind on the chunks texture so the full upload-skip can land. Likely needs an explicit pipeline barrier between regime-1 GPU dispatch and the first render frame, or a different bind-group aliasing strategy.
2. **TAA reprojection of moving entities** (paper §3.6 follow-on) — implement the `entity_instances_history` consumer in `shoot_ray` and flip `ConstructionConfig.entity_history_enabled` to `true`.
3. **Flood-fill cap-28 test coverage** (review nit #2) — add a test that explicitly exercises the cap-28 edge case in `change_handler.rs`.
4. **`render/construction/mod.rs` split** (review nit #3) — split the 3957-line mega-module (`validate_*` functions to a sibling module).
5. **Future shadow-filtering improvements** (user note 2026-05-15, post-TAA-fidelity) — separate later track.
