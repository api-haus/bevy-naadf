...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  ...
  - [~] **Wave 3 — final integration (DISPATCHED 2026-05-15):** wire W4's deferred entity rendering (`NaadfPipelines::world_layout` extension + entity-buffer bindings + `naadf_entity_update_node` body + `shoot_ray` entity sub-traversal invocation) + run all gates on merged main. Single comprehensive integration dispatch.
  - [ ] **Wave 3 — Phase-C review** → `17-review-c.md` fresh-eyes `delegate-reviewer`.
