...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  ...
  - [~] **Wave 2 — foundational (DISPATCHED 2026-05-15):** W1 GPU Algorithm 1 — port `chunkCalc.fx` (3 entry points + `chunk_copy_to_cpu` stub for W4) + `mapCopy.fx` (2 entry points) + `BlockHashingHandler` Rust port + startup regime-1 driver. **THE largest Phase-C workstream.** After W1 merges: W3 ‖ W4 in parallel, then W2.
