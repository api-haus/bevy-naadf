...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  ...
  - [~] **Wave 2 — fan-out (DISPATCHED 2026-05-15 in parallel):** W3 background AADF queue (`bounds_calc.wgsl` regime-2, 5 rounds/frame) ‖ W4 dynamic entities (`entity_update.wgsl` + chunks-texture `R32Uint`→`Rg32Uint` widening + `.x` sweep + per-entity AADF voxel volumes + `shoot_ray` entity sub-traversal). W3 codes against current `R32Uint` chunks; W4 owns the format flip in its atomic merge. Sequencing at merge: W3 first, then W4 (per `15-design-c.md` §2.2). After both: dispatch W2 (editing).
