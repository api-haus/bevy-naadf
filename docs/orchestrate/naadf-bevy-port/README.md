...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  ...
  - [~] **Phase-C followups (DISPATCHED 2026-05-15):** consolidated cleanup — wire `run_gpu_construction_startup` to actually dispatch the regime-1 GPU build chain (the producer flip) + add the missing const-guard for `bounds_common.wgsl` inline duplication + reconcile docs + guard `entity_instances_history` allocation behind a config flag (TAA reprojection of moving entities is Phase-D scope) + calibrate `--entities` pixel-luminance gate + fix the stale `prepare.rs` module doc.
