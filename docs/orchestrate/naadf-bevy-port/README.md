...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  - [~] **Wave 1a (parallel — DISPATCHED 2026-05-15):** W0 seam (`render/construction/` skeleton + `GpuConstructionParams` + chunks texture `STORAGE_BINDING` widening + `--validate-gpu-construction` CLI flag) ‖ W6 O(3·d·n) AADF rewrite (CPU-only neighbour-merge in `aadf/bounds.rs`, ≥10× speedup target). Filesystem-disjoint workstreams; worktree-isolated.
  - [ ] **Wave 1b:** W5 worldgen (depends on W0 merging).
  - [ ] **Wave 2:** W1 GPU Algorithm 1 (depends on W0, W5) → then W3 bounds queue ‖ W4 entities → then W2 editing.
