...
- [~] `impl` phase (Phase C) — team-based parallel implementation across git worktrees per `15-design-c.md` §2. Each workstream writes its impl log to `16-impl-c-W<n>.md`; final integration agent consolidates into `16-impl-c.md`.
  ...
  - [~] **Wave 2 — editing (DISPATCHED 2026-05-15):** W2 `world_change.wgsl` (4 entry points) + CPU `ChangeHandler.UpdateWorld` flood-fill + `EditingHandler` Rust port + `aadf/edit.rs` CPU oracles + `--edit-mode` e2e. Last wave-2 workstream (depends on W3 bound queues for re-enqueueing).
  - [ ] **Wave 3:** final integration merge agent → consolidates into `16-impl-c.md`.
